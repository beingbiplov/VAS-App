...
const ClientRegistrationConfirm: React.FC = () =>{
    ...
    useEffect(() =>{
    if (loggedInUser || !patientData.email){
      navigate('/')  
    }
  })
        ...
        <section className='container formContainer userRegisterConfirmForm'>
            ...
            <Row>
                <Col className='userConfirmFormCol' span={24}>
                    <Title level={5}>First name: <Text type="secondary" italic>{patientData.firstName}</Text></Title>
                    <Title level={5}>Last name: <Text type="secondary" italic>{patientData.lastname}</Text></Title>
                    <Title level={5}>Email: <Text type="secondary" italic>{patientData.email}</Text></Title>
                    <Title level={5}>DOB: <Text type="secondary" italic>{patientData.DOB}</Text></Title>
                    ...
                    <Title level={5}>Address: <Text type="secondary" italic>{patientData.address.street}, {patientData.address.city}, {patientData.address.provience}</Text></Title>
                    <Title level={5}>Payment: <Text type="secondary" italic>{patientData.payment.memberId}, {patientData.payment.insurenceId}, {patientData.payment.insurenceProvider}</Text></Title>
                </Col>
                <Col className='userConfirmFormCol' span={24}>
